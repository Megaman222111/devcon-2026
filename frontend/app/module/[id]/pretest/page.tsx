...
export default async function ModulePretestPage({ params }: PretestPageProps) {
  const { id } = await params
  const moduleMeta = modules.find((module) => module.id === id)
  const moduleNumber = moduleMeta?.number ?? Number(id.replace('mod-', '')) ?? 0
  const test = await loadModuleTest(moduleNumber, 'pretest')

  return (
    <AppShell
      showBottomNav={false}
      breadcrumb={{ module: `Module ${moduleNumber}`, lesson: 'Pre-Test' }}
    >
      <div className="mx-auto max-w-3xl px-4 py-6 pb-24">
        ...
        {test ? (
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-8 text-slate-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200">
            <p>No pre-test markdown found for module {moduleNumber}.</p>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Expected file: <code>files/tests/module_{moduleNumber}_pretest.md</code>
            </p>
          </div>
        )}
      </div>
